...
export class ResponseUtil {
    static getResponse(msg: string, data: any, statusCode: number): APIGatewayProxyResult {
        return {
...
export class JSONUtil {
    ...
    static bufferToJSON(buffer: any) {
        if (buffer) {
            try {
...
export class FilterUtil {
    static parse(filters: Filter[]): ReservationFilter[] {
        return filters.map((filter: Filter) => {
            switch(filter.type) {
                case FilterType.Extras: return new ExtrasFilter(filter.data); 
                case FilterType.Stay: return new StayFilter(filter.data);
            }
        })
    }
}
...
export class SystemLogger { 
    static log(msg: string, data: any = {}, type: LogType = LogType.DEBUG, force: boolean = false) { 
        if (DEBUGGER || force) {
            switch (type) {
